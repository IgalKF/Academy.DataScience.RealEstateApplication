...
import pandas as pd
import re
data_frame = pd.read_csv('../CSV files/allData.csv', index_col= 0)
data_frame.drop_duplicates(inplace=True) # remove duplicates
...
data_frame['עסקאות'] = data_frame.groupby(['יום מכירה','קומה', ' חדרים','גוש חלקה - תת חלקה'])['עסקאות'].transform(lambda x : ' , '.join(x))
